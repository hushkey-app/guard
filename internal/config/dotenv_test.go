...
func TestTheDevelopmentDotEnvIsTheEnvironment(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte("GUARD_RUM_RELEASE=from-dotenv\nGUARD_RUM_SERVICE=web\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	t.Setenv("GUARD_DOTENV", path)
	t.Setenv("GUARD_RUM_RELEASE", "")
	t.Setenv("GUARD_RUM_SERVICE", "")
	set, _ := load(t, nil)

	if got := os.Getenv("GUARD_RUM_RELEASE"); got != "from-dotenv" {
...
func TestARealEnvironmentVariableBeatsTheFile(t *testing.T) {
	path := filepath.Join(t.TempDir(), ".env")
	if err := os.WriteFile(path, []byte("GUARD_RUM_SERVICE=from-the-file\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	t.Setenv("GUARD_DOTENV", path)
	t.Setenv("GUARD_RUM_SERVICE", "from-the-shell")
	load(t, nil)
	if got := os.Getenv("GUARD_RUM_SERVICE"); got != "from-the-shell" {
...
func TestSavingWritesTheFileAndKeepsWhatIsNotGuardsPd(t *testing.T) {
	...
	if _, err := set.Save(map[string]string{"GUARD_MONITOR_INTERVAL": "10s", "GUARD_RUM_RELEASE": "v1.2.3"}); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(written, "GUARD_MONITOR_INTERVAL=10s") || !strings.Contains(written, "GUARD_RUM_RELEASE=v1.2.3") {
	...
	if strings.Count(written, "GUARD_RUM_SERVICE=") != 1 {
		t.Fatalf("a guard line was duplicated:\n%s", written)
	}
	// Written twice, the file does not grow.
	if _, err := set.Save(map[string]string{"GUARD_MONITOR_INTERVAL": "20s"}); err != nil {
		t.Fatal(err)
	}
	...
	if strings.Count(string(again), "GUARD_MONITOR_INTERVAL=") != 1 {
...
func TestAHandWrittenGuardLineSurvivesASave(t *testing.T) {
	path := filepath.Join(t.TempDir(), ".env")
	if err := os.WriteFile(path, []byte("GUARD_RUM_SERVICE=typed-by-hand\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	t.Setenv("GUARD_DOTENV", path)
	t.Setenv("GUARD_RUM_SERVICE", "")
	set, _ := load(t, nil)
	if _, err := set.Save(map[string]string{"GUARD_MONITOR_INTERVAL": "10s"}); err != nil {
		t.Fatal(err)
	}
	raw, _ := os.ReadFile(path)
	if !strings.Contains(string(raw), "GUARD_RUM_SERVICE=typed-by-hand") {
		t.Fatalf("the hand-written line was dropped:\n%s", raw)
	}
	// And once it *is* stored, the stored value is what the file says — one line,
	// not two.
	if _, err := set.Save(map[string]string{"GUARD_RUM_SERVICE": "from-the-page"}); err != nil {
		t.Fatal(err)
	}
	raw, _ = os.ReadFile(path)
	if strings.Count(string(raw), "GUARD_RUM_SERVICE=") != 1 || !strings.Contains(string(raw), "from-the-page") {
