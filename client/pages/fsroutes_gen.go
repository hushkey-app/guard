// Code generated by tools/fsroutes. DO NOT EDIT.
// Source of truth is the directory tree under client/pages.

package pages

import (
	p0 "github.com/mirairoad/guard/client/pages/logs"
	p1 "github.com/mirairoad/guard/client/pages/metrics"
	p2 "github.com/mirairoad/guard/client/pages/settings"
	p3 "github.com/mirairoad/guard/client/pages/settings/cluster"
	p4 "github.com/mirairoad/guard/client/pages/traces"
	p5 "github.com/mirairoad/guard/client/pages/views"
	"github.com/mirairoad/howl-go/core/router"
)

// FsClientRoutes is the route table derived from the page tree.
func FsClientRoutes() []router.Route {
	return []router.Route{
		{Pattern: "/", Label: "Home", Page: Page, Head: Head, Mount: Mount, Unmount: Unmount, Layouts: nil, Client: true, Raw: false},
		{Pattern: "/logs", Label: "Logs", Page: p0.Page, Head: p0.Head, Mount: p0.Mount, Unmount: p0.Unmount, Layouts: nil, Client: true, Raw: false},
		{Pattern: "/metrics", Label: "Metrics", Page: p1.Page, Head: p1.Head, Mount: p1.Mount, Unmount: p1.Unmount, Layouts: nil, Client: true, Raw: false},
		{Pattern: "/settings", Label: "Settings", Page: p2.Page, Head: p2.Head, Mount: p2.Mount, Unmount: p2.Unmount, Layouts: nil, Client: true, Raw: false},
		{Pattern: "/settings/cluster", Label: "Cluster", Page: p3.Page, Head: p3.Head, Mount: p3.Mount, Unmount: p3.Unmount, Layouts: nil, Client: true, Raw: false},
		{Pattern: "/traces", Label: "Traces", Page: p4.Page, Head: p4.Head, Mount: p4.Mount, Unmount: p4.Unmount, Layouts: nil, Client: true, Raw: false},
		{Pattern: "/views", Label: "Views", Page: p5.Page, Head: p5.Head, Mount: p5.Mount, Unmount: p5.Unmount, Layouts: nil, Client: true, Raw: false},
	}
}
